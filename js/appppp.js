...
function showLocalDrafts(el, draftList) {
  ...
  for(var i in localStorage) {
    if ( localStorage[i] !== 'true' && typeof localStorage[i] === 'string' ) {
      ...
      $('.draft-content', draftList).append("<span><a data-draft-id='" + draft.draftId + "' data-company='" + draft.company + "' data-title='" + draft.title + "' data-location='" + draft.location + "' data-cover-letter='" + replaceAllText(draft.coverLetter, "'", "&#39;") + "'>" + companyName + "</a></span>");
      // draftNumber++;
    }
  }
}
